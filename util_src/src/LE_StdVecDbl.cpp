 ...
 */
LE_StdVecDbl::LE_StdVecDbl(const char* lineName, std::vector<double>* vectorDest_ptr,
                           int vecLength, int numRL, const char* varName) :
    LineEntry(lineName, numRL),
    m_vectorDest_ptr(vectorDest_ptr),
    m_CurrentVecValues(0)
{
    ...
    if (numRL != 0 && numRL != 1) {
        throw BI_InputError("LE_StdVecDbl::constructor",
    ...
    if (m_vectorDest_ptr) {
        if ((int)(*m_vectorDest_ptr).size() < vecLength) {
            (*m_vectorDest_ptr).resize(vecLength, 0.0);
        }
    }
    m_CurrentVecValues.resize(vecLength, 0.0);
}


/*
 * LE_StdVecDbl(const LE_StdVecDbl&):
 *
 * copy Constructor:
 *  -> we set the CurrIndex to zero here. And, note, we still
 *     address and write to the same memory locations.
 */
LE_StdVecDbl::LE_StdVecDbl(const LE_StdVecDbl& b) :
    LineEntry(b),
...
/*
 * LE_StdVecDbl& operator=(const LE_StdVecDbl&) :
 *
 *  assignment operator
 */
LE_StdVecDbl& LE_StdVecDbl::operator=(const LE_StdVecDbl& b)
{
    if (&b != this) {
        LineEntry::operator=(b);
        m_vectorDest_ptr = b.m_vectorDest_ptr; // shallow copy, because we are pointing at a shared resource
        MaxVal  = b.MaxVal;
        MinVal  = b.MinVal;
        DefaultVal = b.DefaultVal;
 ...
 */
LineEntry* LE_StdVecDbl::duplMyselfAsLineEntry() const
{
    LE_StdVecDbl* newLE = new LE_StdVecDbl(*this);
 ...
 */
void LE_StdVecDbl::process_LineEntry(const TK_TOKEN* lineArgTok)
{
    int i;
    for (i = 0; i < NumEntryDependencies; i++) {
        ...
        if (idep->ResultType() == BIDRT_USETOPROCESS) {
            ...
            if (idep->checkDepOneInt(value)) {
                ...
                throw BI_InputError("LE_StdVecDbl::process_LineEntry",
...
/*
 * LE_StdVecDbl::print_ProcessedLine() (virtual function):
 *
 ...
 */
void
LE_StdVecDbl::print_ProcessedLine(const TK_TOKEN* lineArgTok) const
{
 ...
 *
 */
void LE_StdVecDbl::set_PrintString(const char* ps)
{
    if (ps) {
...
/*
 * LE_StdVecDbl::adjustAddress() : (virtual)
 *
 ...
 */
void LE_StdVecDbl::adjustAddress(LONG_PTR addrAdjustment)
{
    if (addrAdjustment != 0) {
        if (m_vectorDest_ptr) {
...
const std::vector<double>& LE_StdVecDbl::currentTypedValue() const
{
 ...
 */
const void* LE_StdVecDbl::currentValueAsVoidP() const
{
 ...
 *
 */
bool LE_StdVecDbl::checkRequirements(bool throwSpecificError)
{
    ...
    if (CurrIndex != 0) {
        if (throwSpecificError) {
            throw BI_MissingRequiredVec("LE_StdVecDbl::checkRequirements",
                                        EntryName.orig_str,
                                        VecLength,
                                        CurrIndex);
        } else {
 ...
 *
 */
void LE_StdVecDbl::print_usage(int ilvl) const
{
    print_indent(ilvl);
 ...
 *
 */
void LE_StdVecDbl::set_default(double def)
{
    DefaultVal = def;
 ...
 *
 */
void LE_StdVecDbl::set_limits(double maxV, double minV)
{
    MaxVal = maxV;
    MinVal = minV;
}
