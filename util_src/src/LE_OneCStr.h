...
public:
    ...
    /*!
     * @param addrAdjustment Offset in raw bytes to adjust the internal value of the Address that will receive the string.
     */
    virtual void adjustAddress(LONG_PTR addrAdjustment);

    //! Returns the current value as a pointer to a C string
    /*!
     * This is a nonvirtual function since the return type is specific to this child.
     *
     *  @return           Returns a pointer to a vector consisting of a null-terminated character string.
     *                    This vector shouldn't be modified or freed.
     */
     ...
     *
     *  @return Returns a pointer to current std::string value as a pointer to void.
     */
