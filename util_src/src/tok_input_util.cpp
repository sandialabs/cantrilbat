...
#ifndef SAFE_FREE
//!  Macro to free a pointer and then set its value to zero
...
/*!
 *    If it is inside or on the bounds, it returns false. If it is outside the bounds, it returns true.
 *
 *    @param[in]      value                 Value to be checked
 *    @param[in]      maxVal                Maximum value
 *    @param[in]      minVal                Minimum value
 *
 *    @return                               Returns TRUE if out of bounds, false otherwise
 */
...
//!   and then returns the appropriate answer as an int value in the  variable ret_value.
/*!
 *     It is int because the default value may not  be only 0 or 1
 *
 *    @param[in]      token                 Input string to be interpreted
 *    @param[out]     retn_value            Output BOOLEAN value
 *    @param[in]      defaultVal            Default value
 *
 *    @return                               Returns TRUE if successful or FALSE if unsuccessful.
 */
static BOOLEAN interpret_boolean(const char* token, int* retn_value, const int defaultVal);

//!   This routine interprets a string token to be an integer and then returns the appropriate answer as an int value in the
//!   variable ret_value.
/*!
 *     Errors are indicated by returning FALSE. Success is indicated  by returning TRUE.
 *
 ...
 *
 *    @param[in]      string                Input string to be interpreted
 *    @param[out]     retn_value            Output int value
 *    @param[in]      maxVal                Maximum value
 *    @param[in]      minVal                Minimum value
 *    @param[in]      defaultVal            Default value
 *
 *    @return                               Returns TRUE if successful or FALSE if unsuccessful.
 */
static BOOLEAN interpret_int(const char* string, int* retn_value, const int maxVal, const int minVal, const int defaultVal);

//!   This routine interprets a string token to be a double and then returns the appropriate answer as a double value
//!   in the variable retn_value.
/*!
 ...
 *
 *    @param[in]      string                Input string to be interpreted
 *    @param[out]     retn_value            Output double value
 *    @param[in]      maxVal                Maximum value
 *    @param[in]      minVal                Minimum value
 *    @param[in]      defaultVal            Default value
 *
 *    @return                               Returns TRUE if successful or FALSE if unsuccessful.
 */
static BOOLEAN interpret_double(const char* string, double* retn_value, const double maxVal, const double minVal,
    ...
    nstd_delims_(0)
{
    if (b.orig_str == NULL) {
        ...
        orig_str = copy_string(b.orig_str);
        tok_str  = copy_string(b.orig_str);
        if (b.nstd_delims_) {
            nstd_delims_ = copy_string(b.nstd_delims_);
            ntokes = stokenizeV(tok_str, nstd_delims_, tok_ptrV, MAXTOKENSBIG);
        } else {
            ntokes = stokenizeV(tok_str, DELIMITERS, tok_ptrV, MAXTOKENSBIG);
        }
 ...
 */
{
    ...
    return (str_to_int(tokPtr->tok_ptrV[0], maxVal, minVal, defaultVal, error));
}
//==========================================================================================================================
...
double tok_to_double(const TOKEN* tokPtr, const double maxVal,
                     const double minVal, const double defaultVal, BOOLEAN* error)
/*
 ...
 */
{
    ...
    return (str_to_double(tokPtr->tok_ptrV[0], maxVal, minVal, defaultVal, error));
}
//====================================================================================================================
    ...
    if (interpret_double(dbl_string, &retn_value, maxVal, minVal, defaultVal)) {
        if (check)
            if (retn_value == NO_DEFAULT_DOUBLE) {
                (void) fprintf(stderr, "ERROR: keyLine_double: Default not allowed\n");
                *error = TRUE;
            }
 ...
 */
{
    ...
    return (str_to_boolean(tokPtr->tok_ptrV[0], default_value, error));
}
//============================================================================================================================
 ...
 */
{
    ...
    if (interpret_boolean(string, &retn_value, default_value)) {
        if (retn_value == NO_DEFAULT_BOOLEAN) {
            (void) fprintf(stderr, "ERROR: keyLine_boolean: Default not allowed\n");
            *error = TRUE;
        }
    } else {
        *error = TRUE;
 ...
 *
 *     errorPrinting = true  -> default in .h file.
 *
 *     Output:
...
#else
        if ((retn = sscanf(token, "%le", &retn_dbl)) != 1) {
            *retn_value = (double) retn;
            return (FALSE);
        } else {
            *retn_value = retn_dbl;
 ...
 */
{
    ...
    int  iln = static_cast<int>(strlen(keyptr->orig_str) + 1 + keyptr->ntokes);
    char* fstr = (char*) malloc(iln * sizeof(char));

    char* const* str  = &(keyptr->tok_ptrV[0]);
...
int stokenizeV(char* string, const char* delimiters, std::vector<char*>& tok_ptrV, const int max_tokens)
{
    ...
    tok_ptrV.push_back((char*)0);
    if ((tok_ptrV[0] = strtok(string, delimiters)) != 0) {
        do {
            ++i;
            tok_ptrV.push_back((char*)0);
            if (i == max_tokens) {
        ...
        } while ((tok_ptrV[i] = strtok(NULL, delimiters)) != NULL);
    }
    return (i);
}
//=====================================================================================================================
...
BOOLEAN toktokmatch(const TOKEN* keyptr1, const TOKEN* keyptr2)
{
    ...
    while (i > 0) {
        i--;
        if (!strmatch(keyptr1->tok_ptrV[i], keyptr2->tok_ptrV[i])) {
            return FALSE;
        }
    }
    return TRUE;
 ...
 * toktokincluded()
 *
 *    This routine checks whether the first TOKEN structure is
 *    included in the second TOKEN structure.
...
{
    int i1 = keyptr1->ntokes;
    int i2 = keyptr2->ntokes;
    if (i1 > i2) {
        return FALSE;
    }
    if (i1 == 0) {
    ...
    for (int j = 0; j < i2 - i1 + 1; j++) {
        if (strmatch(keyptr1->tok_ptrV[0], keyptr2->tok_ptrV[j])) {
            istart = j;
            int ifound = 1;
            for (int i = 1; i < i1; i++) {
                if (!strmatch(keyptr1->tok_ptrV[i], keyptr2->tok_ptrV[istart+i])) {
                    ifound = 0;
                    break;
                }
            }
...
void fillTokStruct(TOKEN* keyptr1, const char* s2)
{
    ...
    (keyptr1->tok_ptrV).clear();
    if (s2 == NULL) {
        keyptr1->orig_str = copy_string("");
        keyptr1->tok_str = copy_string("");
        keyptr1->ntokes = 0;
        keyptr1->tok_ptrV.push_back(keyptr1->orig_str);
        return;
    }
 ...
 *
 *    NOTE: Memory leak may result if the calling program doesn't free the malloced space
 *
 *  @param cstr c string to be copied
 *  @return  returns a malloced copy of a string
 */
char* copy_string(const char* const ss)
{
    if (!ss) {
        return (char*)NULL;
    }
    char* new_string  = (char*) malloc(strlen(ss)+1);
    if (new_string == NULL) {
        (void) fprintf(stderr, "copy_string ERROR: malloc returned NULL");
    } else {
        (void) strcpy(new_string, ss);
    }
    return (new_string);
}
//==================================================================================================================================
...
void strip_item_from_token(int iword, TOKEN* tok)
{
    ...
    int ioffset = static_cast<int>(tok->tok_ptrV[iword] - tok->tok_str);
    int ilength = static_cast<int>(strlen(tok->tok_ptrV[iword]));
    int i = ioffset;
    ...
    tok->ntokes = stokenizeV(tok->tok_str, DELIMITERS, tok->tok_ptrV,
                             MAXTOKENS);
}
//==================================================================================================================================
}
