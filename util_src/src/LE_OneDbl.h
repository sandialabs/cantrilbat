/**
 * @file LE_OneDbl.h
 *  Header for the LineEntry of a single double
 *  (see \ref blockentryModule and class
 *  \link BEInput::LE_OneDbl LE_OneDbl\endlink).
 */
 ...
 *                                     &gI.viscosity,
 *                                     reqd, "viscosity");
 *      d2->set_default(1.3E-5);
 *      d2->set_limits(100000., 1.0E-4);
 ...
 *  have exactly one "Viscosity" %LineEntry.
 *
 ...
 *  using the base service request, #ansDepCheck(), and the service
 * request that
 ...
 */
class LE_OneDbl : public LineEntry
{
public:
     ...
     */
    explicit LE_OneDbl(const char* keyName, double* addrVal, int numRL = 0,
     ...
     */
    LE_OneDbl(const LE_OneDbl& right);
     ...
     */
    LE_OneDbl& operator=(const LE_OneDbl& right);

    //! Duplicator function
    /*!
     * This function duplicates the entry and returns a pointer to a LineEntry
     *
     *  @return            Returns a pointer to the duplicate of this card
     */
     ...
     *
     *   @return               Returns the current value of the card as a double
     */
     ...
     *
     *  @return Returns a pointer to current double value as a  pointer to void.
     */
