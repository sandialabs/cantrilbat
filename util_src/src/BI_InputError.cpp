...
namespace BEInput {
  ...
  BI_EndBlockMismatch::BI_EndBlockMismatch(std::string procedure, 
					   ...
					   std::string expected) :
    BI_InputError(procedure, 
		  "\tEnd Block Mismatch: got ->\"" + lineTok + "\"<-"
		  " Expected -> \"" + expected + "\"<-")
  {
  }
  //===========================================================================================================
  ...
  BI_MissingRequiredVec(std::string procedure,
			std::string lineTok,
			int numVneeded, int numVfound) :
    BI_InputError(procedure, 
		  "\tMissing Required Number of vector entries ->\"" 
		  + lineTok + "\"<- :" + integerToStr(numVneeded) + " : " + 
		  integerToStr(numVfound))
  {
  }
