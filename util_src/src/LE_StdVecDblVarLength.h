...
//!  Keyline Entry for a variable vector of doubles which are storred in a std vector.
/*!
 ...
 *      int reqd = 0;
 *      LE_StdVecDblVarLength *v1 = new LE_StdVecDblVarLength("Molar Volumes",
 *                                           &(gI.molarVolumes),
 *                                          gI.numSpecies, reqd,
 *                                          "molarVolumes");
 ...
 *   const vector<double> &molarVols =
 *              *(static_cast<const std::vector<double> *>(le->currentValueAsVoidP()));
 *   int vecLength = le->get_NumTimesProcessed();
 *
 *
 *   LineEntry *le = besmd->searchLineEntry("Molar Volumes");
 *   LE_StdVecDblVarLength *le_dbl = dynamic_cast<LE_StdVecDblVarLength *>(le);
 *   const std::vector<double> &molarVols = le_dbl->currentTypedValue();
 *  @endcode
 *
 *  Using this alternative, we could have set the external address used in
 *  the constructor of <B>%LE_StdVecDblVarLength</B> to 0.
 *
 *
 ...
 */
class LE_StdVecDblVarLength : public LineEntry
{
public:
     ...
     */
    LE_StdVecDblVarLength(const LE_StdVecDblVarLength& right);
     ...
     */
    LE_StdVecDblVarLength& operator=(const LE_StdVecDblVarLength& right);
     ...
     *
     *  @return                       Returns a bool indicating whether the requirements put on this card have been met.
     */
