 ...
 *   class \link BEInput::LE_OneStr LE_OneStr\endlink).
 */
...
//!   The LE_OneStr line entry sets up the line entry special case
 ...
 *    LE_OneStr *sN = new LE_OneStr("Name of the Species", &gI.spName,
 *                                  1, 1, 1, "SpeciesName");
 ...
 *   std::string *value = *(static_cast<const std::string *>(le->currentValueAsVoidP()));
 *
 *   LineEntry *le = besmd->searchLineEntry("Name of the Species");
 *   LE_OneString *le_str = dynamic_cast<LE_OneString *>(le);
 *   std::string value = le_str->currentTypedValue();
 *  @endcode
 *
 *  Using this alternative, we could have set the external address used in
 *  the constructor of <B>%LE_OneString</B> to 0.
 *
 ...
 */
class LE_OneStr : public LineEntry
{
public:
     ...
     *
     *    string spName;
     *    LE_OneStr *sN = new LE_OneStr("Name of the Species", &spName,
     *                                  1, 1, 1, "SpeciesName");
     *    bl->addLineEntry(sN);
     *
     ...
     *
     */
    LE_OneStr(const char* keyName, std::string* addrVal, int maxval = MAXTOKENS,
     ...
     */
    LE_OneStr(const LE_OneStr& right);
     ...
     */
    LE_OneStr& operator=(const LE_OneStr& right);
     ...
     *
     *  @return             Returns the current value as a std::string
     */
    std::string currentTypedValue() const;
     ...
     */
    void set_default(std::string defValue);
...
private:
    ...
    //! External Address of the string that gets updated
    //!  by this command.
    std::string* AddrVal;
