 ...
 *     \link BEInput::LE_OneBool LE_OneBool\endlink).
 */
...
//!  Keyline Entry of a single bool value.
/*!
 ...
 *   bool value = *(static_cast<const bool *>(le->currentValueAsVoidP()));
 *
 *   LineEntry *le = besmd->searchLineEntry("Do Extra Work");
 *   LE_OneBool *le_bool = dynamic_cast<LE_OneBool *>(le);
 *   bool value = le_bool->currentTypedValue();
 *  @endcode
 *
 ...
 *  that
 *  returns an  value, #ansDepCheckOneInt(). The int value returned is the
 *  #CurrValue of the %LE_OneBool object, translated to an int.
 *
 * @ingroup blockentryModule
 *
 */
class LE_OneBool: public LineEntry
{
public:

    //! Main constructor for a boolean keyline entry.
    /*!
     ...
     * @param addrVal   Address of the boolean, external to the
     ...
     */
    explicit LE_OneBool(const char* KeyName, bool* addrVal, int numRL = 0, const char* varName = 0);

    //! Copy constructor
    /*!
     * @param right Object to be copied
     */
    LE_OneBool(const LE_OneBool& right);
     ...
     */
    LE_OneBool& operator=(const LE_OneBool& right);
     ...
     *
     *  @return       Returns the current value as a bool
     */
    bool currentTypedValue() const;
     ...
     */
    void set_default(bool defValue);
