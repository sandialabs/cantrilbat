 ...
 *  Definition for the BlockEntry of a set of doubles that fills up a vector
 *   of molalities 
...
namespace BEInput {
   ...
   */
  BE_MolalityComp::
  BE_MolalityComp(const char *blockName, double **hndlAAA, 
  ...
  /*
   * BE_MolalityComp(const BE_MolalityComp&)
   *
   * Copy constructor
   */
  ...
  /*
   * BE_MolalityComp destructor: (virtual function)
   *
  ...
  void BE_MolalityComp::Wrapup(FILE *ifp_input, const TK_TOKEN *blockArgTok)
  {
    ...
    if (fabs(correct - mSol) > 1.0E-7) {
      throw BI_InputError("BE_MolalityComp::Wrapup",
			  "Solvent molality has been incorrectly overwritten");
    }
  }
