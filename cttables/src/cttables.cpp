			       ...
			       vector<double> U298)
{
    ...
    cout << g.id();
    cout << "\"" << endl;
    dnt(1); cout << "Overall, this is the " << k+1
    ...
    for (int m = 0; m < (int) g.nElements(); m++) {
      ...
      if (na != 0.0) {
	dnt(3); cout << g.elementName(m) << ": " << na <<endl;
      }
    }
    dnt(1); cout << "Electronic Charge = "; pr_dfp(g.charge(k), 2);
    cout << endl;
    double mw = g.molecularWeight(k);
    if (mw == Tiny) { 
      dnt(1); cout << "Molecular Weight = 0 (set to " << Tiny << " for massF conversion)"
		   << " gm/mol" << endl;
    } else {
      dnt(1); cout << "Molecular Weight = " << mw << " gm/mol" << endl;
    }
    if (GTran) {
      tModel = GTran->model();
    }
...
//	GasTransportData td = mt->getGasTransportData(k);
     ...
     */
    dnt(1);
    cout << "Heat of formation (298.15K) = "; pr_dfp(H298[k], 4);
    printf(" %8s\n", UIO.sGibbs.c_str());
    dnt(1);
    double mu_o = H298[k];
    ...
    dnt(1); printf("Minimum Temperature = %g K\n", minTemp);
    dnt(1); printf("Maximum Temperature = %g K\n", maxTemp);
    ...
    for (int i = 0; i < tableWidth-79; i++) printf("-");
    printf("----|\n");
    ...
    for (int i = 0; i < TT.size(); i++) {
      outOfRange = false;
      if (TT[i] + 1.0E-3 < minTemp) outOfRange = true;
      if (TT[i] - 1.0E-3 > maxTemp) outOfRange = true;
      if (outOfRange) {
	cout << "|**";
	pr_df(TT[i], 8, 2);
	cout << "*|";
				  ...
				  vector<double> U298)
{
  ...
  int tableWidth = 72;
  if (haveSpeciesTransportProps) tableWidth += 30;
  if (IOO.IntEnergyColumn) tableWidth += 15;
  if (IOO.ChemPotColumn)   tableWidth += 15;
