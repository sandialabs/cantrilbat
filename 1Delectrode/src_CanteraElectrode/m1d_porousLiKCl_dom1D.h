...
public:
    ...
    virtual void
    SetupThermoShop1(const NodalVars* const nv, const double* const soln_Curr) override;

    //!  Setup shop at a particular point in the domain, calculating intermediate quantites and updating Cantera's objects
    /*!
     ...
     *  All member data with the suffix, _Curr_, are updated by this function.
     *
     *  @param[in]             solnElectrolyte_CurrL  Current value of the solution vector at the left side
     *  @param[in]             solnElectrolyte_CurrR  Current value of the solution vector at the right side
     ...
     *  (note not a virtual function)
     *
     *  @param[in]             xdel                size of the cell
     *  @param[in]             type                Type of call
     *                                                0 - at the left cell boundary
     *                                                1 - at the right cell boundary
     */
    void SetupTranShop(const double xdel, const int type);
     ...
     */
    void updateElectrolyteOld(const double* const solnElectrolyte, const double* const solnDotElectrolyte);


    virtual void updateElectrolyte(const NodalVars* const nv, const double* const solnElectrolyte_Curr) override;
     ...
     */
    void getVoltagesOld(const double* const solnElectrolyte, const double* const solnSolid);

    virtual void getVoltages(const NodalVars* const nv, const double* const solnElectrolyte_Curr) override;
    ...
    virtual void setAtolVector(double atolDefault, const Epetra_Vector_Ghosted& soln,
                               Epetra_Vector_Ghosted& atolVector, const Epetra_Vector_Ghosted* const atolV = 0) override;
