/**
 * @file m1d_EpetraExtras.cpp Utility routines that carry out distributed Epetra operations
 */
...
{
//==================================================================================================================================
Epetra_Comm* Comm_ptr = nullptr;

//==================================================================================================================================
Epetra_Vector* gatherOn0(const Epetra_Vector& distribV, const Epetra_Comm* const acomm_ptr)
{
    // get my procID
    const Epetra_Comm* comm_ptr = acomm_ptr;
    if (acomm_ptr == nullptr) {
        if (Comm_ptr) {
            comm_ptr = Comm_ptr;
        } else {
     ...
     */
    const Epetra_Import* import0 = new Epetra_Import(*e0_map, distribMap);
...
//==================================================================================================================================
void printOn0(const Epetra_Vector& distribV, const Epetra_Comm* const acomm_ptr)
{
    const Epetra_Comm* comm_ptr = acomm_ptr;
    if (acomm_ptr == 0) {
        if (Comm_ptr) {
    ...
    const int procID = comm_ptr->MyPID();
    // Gather it all on proc0
    const Epetra_Vector* e0 = gatherOn0(distribV, acomm_ptr);
    const int numGlobalElements = distribV.GlobalLength();
    if (procID == 0) {
 ...
 */
Epetra_Vector* gatherOnAll(const Epetra_Vector& distribV, const Epetra_Comm* const acomm_ptr)
{
    const Epetra_Comm* comm_ptr = acomm_ptr;
    if (acomm_ptr == 0) {
        if (Comm_ptr) {
     ...
     */
    const int numMyElements = distribV.GlobalLength();
    const int numGlobalElements = numMyElements;
 ...
 *  (an element size of one is assumed)
 */
void gather_nodeV_OnAll(Epetra_Vector& global_node_V, const Epetra_Vector& distrib_node_V, const Epetra_Comm* const acomm_ptr)
{
    const Epetra_Comm* comm_ptr = acomm_ptr;
    if (acomm_ptr == 0) {
        if (Comm_ptr) {
    ...
    if (globalAllMaxGID != distribAllMaxGID) {
        printf("gather_nodeV_OnAll() ERROR proc %d: (global Max GID = %d) != (distrib MAX GID = %d) \n", comm_ptr->MyPID(),
               globalAllMaxGID, distribAllMaxGID);
        AssertTrace(globalAllMaxGID == distribAllMaxGID);
    }

    /*
     *  We assert that the global length of the vector is equal to the local length of the vector multiplied
     *  by the number of processors on all processors. This is the necessary condition for a global all vector.
     *  (HKM -> what about a number of nodes not divisible by the number of processors?)
     */
    const int ng = global_node_V.GlobalLength();
    const int nl = global_node_V.MyLength();
     ...
     *  all of the unknowns from the distributed object onto all of the processors
     */
    const Epetra_Import* importAll = new Epetra_Import(global_node_V.Map(), distribMap);
    /*
...
void gather_nodeIntV_OnAll(Epetra_IntVector& global_node_IV, const Epetra_IntVector& distrib_node_IV, 
                           const Epetra_Comm* const acomm_ptr)
{
    const Epetra_Comm* comm_ptr = acomm_ptr;
    if (acomm_ptr == 0) {
        if (Comm_ptr) {
     ...
     * Assign all elements to all processors
     */
    const int numMyElements = distrib_node_IV.GlobalLength();
    ...
    const int ng = global_node_IV.GlobalLength();
    const int nl = global_node_IV.MyLength();
    AssertTrace(ng == nl * comm_ptr->NumProc());
    /*
     * Create a map with all of the unknowns on all processors
     */
     ...
     *  all of the unknowns from the distributed object onto all of the processors
     */
    const Epetra_Import* importAll = new Epetra_Import(global_node_IV.Map(), distribMap);
...
//
void scatterToAllFrom0(const Epetra_Vector& globalSoln, Epetra_Vector& distribV, const Epetra_Comm* const acomm_ptr)
{
    /*
    ...
    */
     ...
     */
    const Epetra_Import* importAll = new Epetra_Import(distribMap, e0_map);
