...
int
md_read_specific(void * const buf, int bytes, const int source, const int type);

//! Blocking read to the current processor from a source processor
/*!
 *  Sends bytes bytes  of data from the address buf to the destination
 *  processor, dest. The processor will hang on this send until it is
 *  finished.
 *
 ...
 *
 * @param buf     Location of the data
 * @param bytes   Number of bytes of data to be sent
 * @param dest    Destination processor
 * @param type    type ID of the sending process.
 *
...
int
md_wrap_wait_specific(const int sourceC,
                      const int typeC,
