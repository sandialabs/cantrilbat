...
printUsage()
{
    cout << "usage: LiIon_PorousBat [-h] [-help_cmdfile] [-d #] LiIon_PorousBat.inp" << endl;
    cout << "    -h           help" << endl;
...
main(int argc, char** argv)
{
	...
	Epetra_SerialComm Comm;
         ...
         */
        int retn = 0;
        std::string commandFile = "LiIon_PorousBat.inp";
        if (argc > 1) {
            std::string tok;
            for (int j = 1; j < argc; j++) {
                ...
                } else if (commandFile == "" || commandFile == "LiIon_PorousBat.inp") {
                    commandFile = tok;
                } else {
                    printUsage();
                    exit(1);
        ...
        PSinput.AnodeCathodeCompatibility();
         ...
         */
        DomainLayout* dl = new DomainLayout_LiIon_PorousBat(&PSinput);
        ...
        /*
         *  Here is where we actually malloc the electrode objects
         */
        ps->domain_prep();
        ...
        Epetra_Vector& solnInt = t1.solnVector();
...
#ifdef DEBUG_MATRIX_STRUCTURE
        Solve_Type_Enum solveType =SteadyState_Solve;
        ps->residEval(res, false, &solnInt, 0, 0.0, 0.0);
        jac->matrixEval(false, &solnInt, 0, 0.0, 0.0, solveType);
