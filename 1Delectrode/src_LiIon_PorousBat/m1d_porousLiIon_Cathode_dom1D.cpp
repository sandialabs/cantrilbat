 ...
 // needed for displacement calculation. 
  double lastNodePos = 345678;
	...
	// We've turned this off except for the first node, because it is not necessary
	     ...
	     *  we set the flux here ( icurrElectrode_CBR_[] ) to zero. Then, in the surface 
	     *  domain we add the specification of the flux to the residual of the node at the boundary.
             */
	    fluxTright = 0.0;  
	    fluxR_JHPhi = 0.0;
	...
	/*
        ...
        } else if (PS_ptr->energyEquationProbType_ == 4) {
	    ...
	    double icurrElectrode_CBR_CC = -icurrInterface_Cell_[iCell] + icurrElectrode_CBL_[iCell];
            if (residType == Base_ShowSolution || residType == Base_ResidEval) {
	...
	if (residType == Base_ShowSolution) {
		...
		if (iCell == 0) {
			...
			fluxXleft[k] = - resLocal_Species[k];
	...
	if (solidMechanicsProbType_ > 0) {
	    ...
	    ///   *** put fluid pressure induced strain calculation here. 
	    // 	}
	    // } // pressure exists
	  ...
	  // of the stress free strain volume.
	    ...
	    if(iCell == 0 ) {
	      lastNodePos = soln[indexCent_EqnStart + nodeTmpsCenter.Offset_Displacement_Axial] 
		+ nodeCent->x0NodePos()
		-res[indexCent_EqnStart + nodeTmpsCenter.Offset_Displacement_Axial] ;
	      if(pMECH_MODEL_extra) std::cout << " cc "<<iCell <<" last pos recovered "<<lastNodePos<<" Eyoung "<<Eyoung<<std::endl;
	    }
	    else {
	...
	if(pMECH_MODEL_extra) std::cout << " cath "<<iCell<<" "<<lastNodePos<<
				" cellS "<<cellStrain<<" last "<<lastNodePos<<" nLpos "<<nodeLeft->x0NodePos()<<" xr "<<xratio[iCell-1]<<" strainednp "<<strainedNodePos<<std::endl;
...
void
porousLiIon_Cathode_dom1D::eval_SpeciesElemBalance(const int ifunc,
						   const double t,
						   const double deltaT,
						   const Epetra_Vector *soln_ptr,
						   const Epetra_Vector *solnDot_ptr,
						   ...
						   class globalHeatBalVals& dVals)
{
	...
	for (int iCell = 0; iCell < NumLcCells; iCell++) {
		...
		moleFluxRight = Fright_cc_ * concTot_Curr_;
		for (size_t k = 0; k < (size_t) nsp_; k++) {
		    convRight[k] = Fright_cc_ * mfElectrolyte_Thermo_Curr_[k] * concTot_Curr_;
		}
		for (size_t k = 0; k <  (size_t) nsp_; k++) {
		...
		elem_Lyte_New[k] += elem_Lyte_New_Cell(k, iCell);
		elem_Lyte_Old[k] += elem_Lyte_Old_Cell(k, iCell);
		...
		if (itimes == 0) {
			...
			if (iCell == 0) {
			    residAdd = deltaT *
			      leftD_sep->DomainResidVectorRightBound_LastResid_NE[nodeTmpsCenter.RO_Species_Eqn_Offset + k];
			}
			if (iCell == NumLcCells - 1) {
			    //residAdd = deltaT *
        ...
        for (int vph = 0; vph < numVolPhasesE; vph++) {
            ...
            if (iph == metalPhase || iph == solnPhase) {
                ...
                for (int k = 0; k < nspPhase; k++) {
                    ...
                    fprintf(ofp, "\"MoleDens %s [kmol/m^3]\" \t", sss.c_str());
                }
            }
        }
...
#else
	fprintf(ofp, "ZONE T = \"CATH, %10.4E\", I = %d, SOLUTIONTIME = %12.6E\n", t, numNodes, t);
	fprintf(ofp, "ZONETYPE = ORDERED\n");
	fprintf(ofp, "DATAPACKING = BLOCK\n");
	fprintf(ofp, "STRANDID = 3\n");
        // ----------------------------------------------------------------------------------------------------------------------
