                                       ...
                                       const Solve_Type_Enum solveType)
{
  ...
  for (int iCell = 0; iCell < NumLcCells; iCell++) {
	...
	if(nodeRight) 
      ...
      // *** calculate fluid pressure effects here. 
      ...
      if(iCell == 0 ) {
	// recover lastNodePos from residual
	...
	if(pMECH_MODEL_extra) std::cout << " c "<<iCell<<" "<<lastNodePos<<std::endl;
      }
      else {
	// note that iCell==0, residual for displacement has already been calculated in the Anode code. 
	double cellStrain = soln[indexCent_EqnStart + nodeTmpsCenter.Offset_Solid_Stress_Axial]/Eyoung;
	double strainedNodePos = lastNodePos +(nodeCent->x0NodePos() - nodeLeft->x0NodePos())*xratio[iCell-1]*(1+cellStrain);
	...
	if(pMECH_MODEL_extra) std::cout << " slnp "<<iCell<<" "<<lastNodePos<<
