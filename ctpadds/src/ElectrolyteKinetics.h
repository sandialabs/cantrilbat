/**
 * @file ElectrolyteKinetics.h
 *
 */
/*
 * $Id: ElectrolyteKinetics.h 508 2013-01-07 22:54:04Z hkmoffa $
 */
/*
...
#ifndef ELECTROLYTEKINETICS_H
#define ELECTROLYTEKINETICS_H
...
#endif 
{
  ...
  const int cElectrolyteKinetics = 8999;
  // forward references
  ...
  public:
    ElectrolyteKineticsData() :
      m_logC0_scalar(0.0),
      m_ROP_ok(false), 
      m_temp(0.0)
    {
    }
    virtual ~ElectrolyteKineticsData(){}
  ...
  /**
   * Kinetics manager for elementary liquid phase  chemistry. This
   * kinetics manager implements standard mass-action reaction rate
   * expressions for condensed phases.
   */
  class ElectrolyteKinetics : public Kinetics {
  ...
  public:
    ...
    /// Constructor.
    ElectrolyteKinetics();
    ElectrolyteKinetics(thermo_t* thermo);
      

    /// Destructor.
    virtual ~ElectrolyteKinetics();

    /**
     * Return the ID number for this class
     */
    virtual int ID() const { return cElectrolyteKinetics; }
     ...
     *
     * @param netROP[i] On return it will contain the forward rate
     *                  of progress
     ...
     * reaction [kmol/m^3 s^1]. 
     *
     * @param netROP[i] On return it will contain the reverse rate 
     *                 of progress
     *                  of the ith reaction [kmol/m^3 s^1]. Dimensioned
     *                  at least m_ii.
     */
     ...
     */
    virtual void getRevRateConstants(doublevalue *krev, 
				     bool doIrreversible = false);
    virtual void getActivationEnergies(doublevalue *E);
  ...
  protected:
     ...
     *  coefficient of ns.
     */
    mutable std::vector<std::map<int, doublevalue> >     m_rrxn;
     ...
     * each class instance.
     */
    ElectrolyteKineticsData* m_kdata;
  ...
  private:
     ...
     * constants.
     * The inverse of the equilibrium constant is located internally in m_rkcn[].
     */
    void updateKc();
    ...
    void applyButlerVolmerCorrection(doublevalue* kf);
