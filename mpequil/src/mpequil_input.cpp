...
MPEQUIL_KEY_INPUT PO;
/*************************************************************************
 * MPEQUIL_KEY_INPUT(): constructor
 */
MPEQUIL_KEY_INPUT::MPEQUIL_KEY_INPUT () :
    CanteraFN1("gas.cti"),
 ...
 *            2 = output
 */
bool process_input(BlockEntry *cf, std::string fileName, int printFlag) {
    static int pass = 0;
    pass++;
    cf->ZeroLineCount();
    const TOKEN tok_in;
...
/**************************************************************************
 *
 *  MPEQUIL_CANTERAPROB_INPUT:destructor
 *
 ...
 */
int
mpequil_input(MPEQUIL_INPUT *pi, std::string commandFile)
{
   int   i, k, e;
   ...
   if (specifiedSpeciesMoles) {
     ...
     for (iphase = 0; iphase < pi->nphase; iphase++) {
       ThermoPhase *tPhase = &(mp->phase(iphase));
       pi->phaseMoles[iphase] = 0.0;
       double phaseMolesEst = 1.0;
       for (k = 0; k < (int) tPhase->nSpecies(); k++) {
	 pi->spMoles[kT] = phaseMolesEst * tPhase->moleFraction(k);
	 pi->phaseMoles[iphase] +=  pi->spMoles[kT];
	 for (e = 0; e < pi->ne; e++) {
	   double natoms = mp->nAtoms(kT, e);
	   pi->elementMoles[e] += natoms * pi->spMoles[kT];
	 }
	 kT++;
       }
     }
