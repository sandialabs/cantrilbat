...
int main(int argc, char** argv) {
    ...
    try {
      ...
      FILE *ip = fopen("out.csv", "w");
      fprintf(ip, "time ,   growthRate, ");
      for (int i = 0; i < nsp_d100; i++) {
	string sss = diamond100TP->speciesName(i);
	fprintf(ip, " %s ,", sss.c_str());
      }
      fprintf(ip, "\n");
	...
	time += deltaT;
	printf(" TIME = %g\n", time);
	iKin_ptr->advanceCoverages(deltaT);
	...
	fprintf(ip, "%g, %g, ", time, gr);
	for (k = 0; k <  nsp_d100; k++) {
	  fprintf(ip, "%g, ",  x[k]);
	}
	fprintf(ip, "%\n");
