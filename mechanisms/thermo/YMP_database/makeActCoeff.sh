...
then
  ...
  ln -s  data0.ypf.R2_betas_ca_rev1.xml beta_cmx.xml
fi

if test ! -e  lambda_nj.xml 
then
  ln -s  data0.ypf.R2_extract_Pitzer_params_lambdas_rev2.xml lambda_nj.xml 
fi


if test ! -e    psi_cca_caa.xml
then
  ln -s data0.ypf.R2_extract_Pitzer_params_psi_rev2.xml  psi_cca_caa.xml
fi
