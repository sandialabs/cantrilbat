...
//==================================================================================================================================
static int getThetaCationParamXML(XML_Node* acNode, std::vector<XML_Node*>& ccc)
{
    acNode->getChildren("thetaCation", ccc);
    int sz = ccc.size();
    return sz;
}
//==================================================================================================================================
static int getThetaAnionParamXML(XML_Node* acNode, std::vector<XML_Node*>& ccc)
{
    acNode->getChildren("thetaAnion", ccc);
    int sz = ccc.size();
    return sz;
}
//==================================================================================================================================
...
{
    std::cout << "usage: xmlPitzerAC_Theta [-h] [-help_cmdfile] [fileName]  "
              <<  std::endl;
    std::cout << "    -h           help" << endl;
    std::cout << "    fileName:   XML File to parse for theta blocks" << endl;
    std::cout << std::endl;
}
//==================================================================================================================================
int main(int argc, char* argv[])
{
    ...
    FILE* fcsv = fopen("ThetaParam.csv", "w");

    // Buffer the csv line into buf;
    for (size_t ii = 0; ii < 2; ++ii) {
        std::string itype = "thetaCation";
        if (ii == 0) {
            numParamBlocks = getThetaCationParamXML(acNode, ccc);
            printf("Got  %d binary theta cation parameter points\n", (int) numParamBlocks);
        } else if (ii == 1) {
            itype = "thetaAnion";
            numParamBlocks = getThetaAnionParamXML(acNode, ccc);
            printf("Got  %d binary theta anion parameter points\n", (int) numParamBlocks);
        }
        for (size_t iP = 0; iP < numParamBlocks; iP++) {
            ...
            if (ii == 0) {
                ...
                spName1 = bspNode->attrib("anion1");
                spName2 = bspNode->attrib("anion2");
            }
            if (spName1 == "") {
            ...
            if (bspNode->hasChildOnlyOne("theta")) {
                num = ztml::getFloatArray(*bspNode, thetaB, "", "", "theta");
                if ((int) thetaB.size() != num) {
                    printf("Theta block for  %s  %s  return and size differs: %d %d\n", spName1.c_str(), spName2.c_str(), num,
                           (int) thetaB.size());

                }
            } else {
                printf("Theta block for  %s  %s  not there\n", spName1.c_str(), spName2.c_str());
                exit(-1) ;
            }
            if (thetaB.size() != 5) {
                printf("Theta block for  %s  %s  only has %d entries\n", spName1.c_str(), spName2.c_str(), num);
                exit(-1) ;
            }
            if (thetaB[2] != 0.0) {
                printf("theta[2] != 0.0: %g\n", thetaB[2]);
                exit(-1);
            }
            sprintf(buf + slen, "% .2f , %.2f , %.2f , %.2f , ", thetaB[0], thetaB[1], thetaB[3], thetaB[4]);
            slen = strlen(buf);
